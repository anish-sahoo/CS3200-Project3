...
const cacheAllItemsAndPrices = async () => {
  try {
    ...
    for (const item of allItems) {
      await redisClient.setEx(`item:${item.item_id}`, 3600, JSON.stringify(item.prices)); // Cache for 1 hour
    }
...
const getItemPrices = async (itemId, callback) => {
  ...
  if (results) {
    ...
    if (item) {
      ...
      await redisClient.setEx(`item:${itemId}`, 3600, JSON.stringify(item.prices)); // Cache for 1 hour
...
app.put("/api/items/:itemId/prices/:storeId", async (req, res) => {
  ...
  try {
    ...
    const item = await db.collection("items").findOne({ item_id: Number(itemId) });
    ...
    const storeIndex = item.prices.findIndex((price) => String(price.store.store_id) === String(storeId));
    ...
    const updatedItem = await db.collection("items").findOne({ item_id: Number(itemId) });
    console.log("Updated item", updatedItem);
    // Cache the updated item in Redis
    redisClient.setEx(`item:${itemId}`, 3600, JSON.stringify(updatedItem.prices)); // Cache for 1 hour
